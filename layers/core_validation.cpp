...
static bool verify_renderpass_compatibility(layer_data *my_data, const VkRenderPass primaryRP, const VkRenderPass secondaryRP,
                                            string &errorMsg) {
    auto primaryRenderPass = getRenderPass(my_data, primaryRP);
    auto secondaryRenderPass = getRenderPass(my_data, secondaryRP);
    ...
    const VkRenderPassCreateInfo *primaryRPCI = primaryRenderPass->pCreateInfo;
    const VkRenderPassCreateInfo *secondaryRPCI = secondaryRenderPass->pCreateInfo;
...
static bool validateRenderPassCompatibility(layer_data *dev_data, VkCommandBuffer primaryBuffer, VkRenderPass primaryPass,
    ...
    if (primaryPass == secondaryPass)
        return skip_call;
    auto primaryRenderPass = getRenderPass(dev_data, primaryPass);
    auto secondaryRenderPass = getRenderPass(dev_data, secondaryPass);
    if (!primaryRenderPass) {
        skip_call |=
            log_msg(dev_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, (VkDebugReportObjectTypeEXT)0, 0, __LINE__,
                    DRAWSTATE_INVALID_SECONDARY_COMMAND_BUFFER, "DS",
    ...
    if (primaryRenderPass->pCreateInfo->subpassCount != secondaryRenderPass->pCreateInfo->subpassCount) {
        skip_call |= log_msg(dev_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, (VkDebugReportObjectTypeEXT)0, 0, __LINE__,
                             DRAWSTATE_INVALID_SECONDARY_COMMAND_BUFFER, "DS",
                             "vkCmdExecuteCommands() called w/ invalid Cmd Buffer 0x%p which has a render pass 0x%" PRIx64
                             " that is not compatible with the current render pass 0x%" PRIx64 "."
    ...
    auto subpassCount = primaryRenderPass->pCreateInfo->subpassCount;
    for (uint32_t i = 0; i < subpassCount; ++i) {
        skip_call |=
            validateSubpassCompatibility(dev_data, primaryBuffer, primaryRenderPass,
                                         secondaryBuffer, secondaryRenderPass, i, subpassCount > 1);
    }
    return skip_call;
}
